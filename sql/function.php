...
class BD_con
{
    ...
    ## insert User Member
    public function insert_member($randomuserMember,$username, $password, $gender, $birthday, $email , $image_path ,$device_id)
    {
        $result = mysqli_query($this->dbcon, "INSERT INTO `member`(`code_member`,`username`, `password`, `gender`, `birthday`, `email`, `status`, `create_date`, `image_path`,`device_id`) 
        VALUES ('$randomuserMember','$username',MD5('$password'),'$gender','$birthday','$email','1',Now(),'$image_path','$device_id')"); 
        return $result;
    }
    ## insert User Member
    ...
    public function insert_favorite($id_member, $id_station, $status)
    {
        $result = mysqli_query($this->dbcon, "INSERT INTO `favorite`(`id_member`, `id_station`, `status`) 
        VALUES ('$id_member','$id_station','$status')");
        return $result;
    }
    ## insert User favorite
    ...
    public function insert_image_station($target_dir,$station_name,$id_station,$id_member)
    {
        $result = mysqli_query($this->dbcon, "INSERT INTO `image_station`(`file_path`, `file_name`, `create_date`, `id_station`,`id_member`) 
        VALUES ('$target_dir','$station_name',Now(),'$id_station','$id_member')");
        return $result;
    }
    ## insert User image_station
    ...
    public function insert_station($station_name, $location, $description ,$provider ,$charge_type ,$service_period ,$facilities,$sqlInsertMap)
    {
        ...
        $map_station = mysqli_insert_id($this->dbcon);
        return $map_station;
    }
    ## insert User Station
    // ## INSER DATA END
    ...
    {
        $result = mysqli_query($this->dbcon, "SELECT station.id_station,station.station_name,history_car.percent_start,history_car.percent_stop,history_car.charge_time FROM history_car,car,station WHERE car.id_car = '$id_car' AND history_car.id_car = car.id_car AND station.id_station=history_car.id_station");
        return $result;
    }
    ...
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM favorite,station,image_station WHERE 1 AND favorite.id_member='$id_member' ");
        return $result;
    }
    
    public function select_map()
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM map,station WHERE  station.id_map = map.id_map");
        return $result;
    }

    public function select_station($id_station)
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM station WHERE id_station = '$id_station'");
        return $result;
    }

        public function select_station_all()
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM station");
        return $result;
    }

    public function select_image_station($id_station)
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM image_station WHERE id_station = $id_station");
        return $result;
    }

    public function select_station_and_image($id_station)
    {
        $result = mysqli_query($this->dbcon, "SELECT * FROM station,image_station WHERE station.id_station = image_station.id_station AND station='$id_station'");
        return $result;
    }
